...
updateAuthor index author@D.Author {..} = case getIdent of
  ...
  Nothing -> revertMuidIfNeeded
  where
    getIdent :: Maybe LText
    getIdent = from <$> getIdentByAlias index (from authorUid)
    revertMuidIfNeeded :: D.Author
    revertMuidIfNeeded =
      let muid =
            -- Remove the host prefix
            T.drop 1 $ T.dropWhile (/= '/') (from authorUid)
       in if authorMuid == from muid
            then author
            else D.Author (from muid) authorUid

-- | Apply identities according the configuration on Events
...
updateIdentsOnChanges = do
  ...
  go indexName (updateAuthor index)
  where
    go indexName updateAuthor' = do
