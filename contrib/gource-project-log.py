...
def main():
    ...
    for entry in data:
        ...
        print('%.0f|%s|A|/%s/%s|' % (created_time.timestamp(), entry['author'],
                                     entry['repository_fullname'],
                                     entry['title']))
        print('%.0f|%s|M|/%s/%s|' % (merge_time.timestamp(), entry['author'],
                                     entry['repository_fullname'],
                                     entry['title']))
    ...
    for entry in data:
        ...
        try:
            ...
            print('%s not merged' % entry['repository_fullname_and_number'],
                  file=sys.stderr)
